...
def convert_to_elowen_sections(
    html: str, placeholder_map: Dict[str, ElowenContent], strip_double_brackets=False
) -> List[ElowenSection]:
    """
    Converts an HTML string into a hierarchical list of ElowenSection objects.
    ...
    for tag in soup.recursiveChildGenerator():
        ...
        # Process content tags (p, ul, ol, etc.)
            ...
            if tag.name in DEFAULT_TEXT_TAGS:
                new_contents: List[ElowenContent] = _parse_html_block_for_elowen_contents(
                    get_text(tag),
                    tag.name,
                    placeholder_map,
...
def _parse_html_block_for_elowen_contents(
    text: str,
    ...
    """
    Parses a raw HTML string (e.g., from tag.decode_contents()) into a sequence of ElowenContent objects,
    ...
    elowen_contents: List[ElowenContent] = []
    ...
    for match in placeholder_pattern.finditer(text):
        ...
        if match.start() > current_pos:
            ...
            if pre_text.strip():
                ...
                spans = create_elowen_spans(
                    cleaned_text,
                    inner_tags,
                    strip_double_brackets=strip_double_brackets,
                )
                if spans:
                    elowen_contents.append(
                        ElowenContent(
                            id=get_unique_id(),
        ...
        if placeholder_id in placeholder_map:
            elowen_contents.append(placeholder_map[placeholder_id])
