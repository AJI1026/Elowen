...
from shared.elowen_doc import ElowenContent
...
def parse_elowen_import(model_output_string: str) -> dict:
    """
    Parses a markdown string formatted with elowen-specific tags into a dictionary.
...
def substitute_equation_placeholders(
    text: str, placeholder_map: Dict[str, ElowenContent]
) -> str:
    """
...
def postprocess_content_text(text: str, strip_double_brackets=False) -> str:
    """
    Post-processes text right before it is finalized in a ElowenSpan
    ...
    text = text.replace("\\$", "$")
    # (2) Removing any remaining Elowen tags that were not correctly processed.
...
# Models sometimes emit [[spanId]] instead of [[l-sref-spanId]].
_BARE_SPAN_REF_PATTERN = re.compile(r"\[\[(?!l-)([^\]]{1,64})\]\]")
...
def normalize_bare_span_refs(text: str) -> str:
    ...
    def _replace(match: re.Match) -> str:
        ...
        if not re.fullmatch(r"[A-Za-z0-9_.:-]+", inner):
            return match.group(0)
        return f"{import_tags.S_REF_START_PREFIX}{inner}{import_tags.S_REF_END}"

    return _BARE_SPAN_REF_PATTERN.sub(_replace, text)