...
} from "./elowen_doc_utils";
import { InnerTagName, ElowenContent, ElowenSection, ElowenSpan } from "./elowen_doc";

// Helper to create a mock ElowenSpan with spanref tags
const createSpanWithRefs = (id: string, refIds: string[]): ElowenSpan => ({
  id,
  text: `span ${id}`,
...
// Helper to create a mock ElowenSpan without spanref tags
const createSpanWithoutRefs = (id: string): ElowenSpan => ({
  id,
  text: `span ${id}`,
  innerTags: [],
});

// Helper to create a mock ElowenContent with spans
const createContentWithSpans = (spans: ElowenSpan[]): ElowenContent => ({
  id: "content-id",
  textContent: {
    tagName: "p",
    spans: spans,
...
// Helper to create a mock ElowenContent
const createMockContent = (id: string): ElowenContent => ({
  id,
  textContent: {
    tagName: "p",
...
describe("getAllSpansFromContents", () => {
  const span1: ElowenSpan = { id: "s1", text: "one", innerTags: [] };
  const span2: ElowenSpan = { id: "s2", text: "two", innerTags: [] };
  const span3: ElowenSpan = { id: "s3", text: "three", innerTags: [] };
  const span4: ElowenSpan = { id: "s4", text: "four", innerTags: [] };
  const span5: ElowenSpan = { id: "s5", text: "five", innerTags: [] };
