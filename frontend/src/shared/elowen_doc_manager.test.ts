...
// Helper to create a mock ElowenSpan
const createSpan = (id: string, text: string): ElowenSpan => ({
  id,
  text,
  innerTags: [],
});
...
const createTextContent = (spans: ElowenSpan[]): ElowenContent => ({
...
const createListContent = (items: ListItem[]): ElowenContent => ({
  id: `content-${Math.random()}`,
  textContent: null,
  imageContent: null,
  htmlFigureContent: null,
...
// Helper to create a mock ElowenDoc
const createDoc = (
  abstractContents: ElowenContent[],
...
describe("ElowenDocManager", () => {
  ...
  it("should return the original ElowenDoc object via the elowenDoc getter", () => {
    const doc = createDoc([], [], []);
    const manager = new ElowenDocManager(doc);
    expect(manager.elowenDoc).to.deep.equal(doc);
  });
