...
import "../components/elowen_doc/elowen_doc";
import { InnerTagName, ElowenDoc, LoadingStatus } from "../shared/elowen_doc";
...
const PAPERS: { [key: string]: ElowenDoc } = {
...
const meta = {
  title: "Components/ElowenDoc",
  tags: ["autodocs"],
  render: (args) => {
    const elowenDocManager = new ElowenDocManager(args.elowenDoc);
    const collapseManager = new CollapseManager(elowenDocManager);
...
export const UsingImportedPaper: Story = {
  ...
  render: (args) => {
    ...
    return meta.render({ ...args, elowenDoc });
  },
};
