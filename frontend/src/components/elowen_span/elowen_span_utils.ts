...
import { InnerTag } from "../../shared/elowen_doc";
 ...
 * adjusting their positions to be relative to the root `ElowenSpan`'s text
...
export function getSpanHighlightsFromManagers(
  ...
  userHighlightManager?: UserHighlightManager
) {
  const highlights = [];
  if (highlightManager) {
  ...
  if (userHighlightManager) {
    highlights.push(...userHighlightManager.getSpanHighlights(spanId));
  }
